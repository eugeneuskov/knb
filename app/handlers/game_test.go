...
func TestGameNewGame(t *testing.T) {
	...
	existingAuthToken, err := layers.service.Security.GenerateAuthToken(uuid.MustParse(fixtures.AlreadyExistPlayer1Uuid))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tCase := range gameNewGameSuccessTestCases {
		t.Run(tCase.name, func(tt *testing.T) {
			...
			assert.Equal(tt, fixtures.AlreadyExistPlayer1Uuid, game.Players[0].ID.String())
		})
	}
...
func TestGameJoinGame(t *testing.T) {
	...
	gameJoinGameFailedTestCases := []gameJoinGameTestCase{
		...
		},
		{
			...
			gameId: fixtures.AlreadyExistGameUuid,
	...
	playerAuthToken, err := layers.service.Security.GenerateAuthToken(uuid.MustParse(fixtures.AlreadyExistPlayer2Uuid))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tCase := range gameJoinGameSuccessTestCases {
		t.Run(tCase.name, func(tt *testing.T) {
			...
			assert.Equal(tt, fixtures.AlreadyExistPlayer1Uuid, game.Players[0].ID.String())
			assert.Equal(tt, fixtures.AlreadyExistPlayer1Uuid, response.Players[0].ID.String())
			assert.Equal(tt, fixtures.AlreadyExistPlayer1DisplayName, response.Players[0].Name)
			assert.Equal(tt, fixtures.AlreadyExistPlayer2Uuid, game.Players[1].ID.String())
			assert.Equal(tt, fixtures.AlreadyExistPlayer2Uuid, response.Players[1].ID.String())
			assert.Equal(tt, fixtures.AlreadyExistPlayer2DisplayName, response.Players[1].Name)
		})
	}
